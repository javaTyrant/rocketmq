...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
    ...
    //消费组的名称，在 RocketMQ 中，对于消费者来说，一个消费组就是一个独立的隔离单位，
    //例如多个消费组订阅同一个主题，其消息进度（消息处理的进展）是相互独立的，两者不会有任何的干扰。
    /**
     ...
     * 消息队列负载算法。主要解决的问题是消息消费队列在各个消费者之间的负载均衡策略，
     * 例如一个 Topic 有８个队列，一个消费组中有３个消费者，那这三个消费者各自去消费哪些队列。
     * RocketMQ 默认提供了如下负载均衡算法：
     * 1.AllocateMessageQueueAveragely：平均连续分配算法。
     * 2.AllocateMessageQueueAveragelyByCircle：平均轮流分配算法。
     * 3.AllocateMachineRoomNearby：机房内优先就近分配。
     * 4.AllocateMessageQueueByConfig：手动指定，这个通常需要配合配置中心，在消费者启动时，首先先创建 AllocateMessageQueueByConfig 对象，
     * 然后根据配置中心的配置，再根据当前的队列信息，进行分配，即该方法不具备队列的自动负载，在 Broker 端进行队列扩容时，无法自动感知，需要手动变更配置。
     * 5.AllocateMessageQueueByMachineRoom：消费指定机房中的队列，该分配算法首先需要调用该策略的 setConsumeridcs(Set<String> consumerIdCs) 方法，
     * 用于设置需要消费的机房，将刷选出来的消息按平均连续分配算法进行队列负载。
     * 6.AllocateMessageQueueConsistentHash 一致性 Hash 算法。
     */
    ...
    /**
     * Message listener,消息监听者
     */
    private MessageListener messageListener;

    /**
     * Offset Storage,消息进度存储管理器
     */
